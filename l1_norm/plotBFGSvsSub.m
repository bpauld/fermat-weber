...
[x,k,stop] = bfgs(A,omega,x0,c1,c2,eps1,eps2) ;

f_bfgs = zeros(nbOfValues,1) ;
first_f_bfgs = bfgs_values(A,omega,x0,c1,c2,k) ;
for i=1:k
...
f_sub = subgradient_values(A,omega,nbOfValues) ;

figure
%plot(x_axis,f_bfgs,'--',x_axis,f_sub) ;
semilogx(x_axis,f_bfgs,'--',x_axis,f_sub) ;
title("Convergence for l-1 norm")
xlabel('Iteration')
